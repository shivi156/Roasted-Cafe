...
public class Checkout : PageModel
{
    ...
    public async Task<IActionResult> OnPostIncrementQuantityAsync(int itemID)
    {
        ...
        var basketItem =
            _db.BasketItems
                .FromSqlRaw("SELECT * From BasketItems " +
                            "WHERE BasketID = {0} and StockID = {1}", customer.BasketID, itemID)
                .FirstOrDefault();
    ...
    public async Task<IActionResult> OnPostDecrementQuantityAsync(int itemID)
    {
        var user = await _UserManager.GetUserAsync(User);
        CheckoutCustomer customer = await _db
            .CheckoutCustomers
